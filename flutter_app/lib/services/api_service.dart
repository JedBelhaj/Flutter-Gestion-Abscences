	...
	Future<http.Response> _requestWithFallback({
		...
		for (final base in candidates) {
			try {
					...
					final uri = baseUri.replace(queryParameters: queryParameters.isEmpty ? null : queryParameters);
				final headers = <String, String>{
					'Accept': 'application/json',
						if (method == 'POST' || method == 'PUT' || method == 'DELETE') 'Content-Type': 'application/json',
					...
					final response = switch (method) {
						...
						'DELETE' => await http.delete(uri, headers: headers, body: jsonEncode(body ?? {})),
						_ => throw UnsupportedError('Unsupported HTTP method: $method'),
					};

				if (_looksLikeHtml(response.body) && !_looksLikeJson(response.body)) {
					lastError = Exception('Reponse HTML pour $uri (HTTP ${response.statusCode})');
					continue;
				}
